...
PATTERN='captain|crewmate|crewmates|matey|ahoy|shipshape|yarr|nautical'

if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  FILES=$(git ls-files --cached --others --exclude-standard | grep -Ev '^scripts/lint-vocab\.sh$' || true)
else
