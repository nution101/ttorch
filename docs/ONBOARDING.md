...
| `TTORCH_STALL_AFTER` | `10m` | how long an active worker may sit at an unchanging idle prompt before `ttorch watch` raises a `stalled` update (minimum `1m`); `0`/`off` disables the stall ladder |
