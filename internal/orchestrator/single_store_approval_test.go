...
func expireToken(t *testing.T, m *Manager, id, by, sha string) {
	t.Helper()
	if err := approval.Grant(m.P.ApprovalFile(id), -time.Second, approvalPayload(by, sha, nil)); err != nil {
		t.Fatal(err)
	}
...
func TestLand_ExpiredHumanTokenLandsNoAgeBound(t *testing.T) {
	...
	if _, err := m.Approve("eh1", time.Minute, false); err != nil { // human token pinned to head
		t.Fatal(err)
	}
...
func TestMergeLocal_OverAgeAutoMintRefusedWithEvent(t *testing.T) {
	...
	if _, err := m.Approve("oa1", time.Minute, false); err != nil {
		t.Fatal(err)
	}
