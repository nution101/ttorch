...
func TestFreshDBBootstrapsToV0(t *testing.T) {
	...
	if v, err = s.schemaVersion(ctx); err != nil || v != 9 {
		t.Errorf("after Migrate: version=%d err=%v, want 9/nil", v, err)
	}
}
...
func TestMigrateUpDownUp(t *testing.T) {
	...
	if v, err := s.schemaVersion(ctx); err != nil || v != 9 {
		t.Fatalf("after Open: version=%d err=%v, want 9", v, err)
	}
	var fk int
	...
	if v, err := s.schemaVersion(ctx); err != nil || v != 9 {
		t.Fatalf("after re-up: version=%d err=%v, want 9", v, err)
	}
...
func TestMigrateIdempotent(t *testing.T) {
	...
	if rows != 9 {
		t.Errorf("schema_migrations rows = %d, want 9 (no duplicate ledger row)", rows)
	}
}
